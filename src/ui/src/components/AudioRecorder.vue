<!--
  Meowsformer — AudioRecorder Component (Legacy, root-level copy)
  See components/translate/AudioRecorder.vue for the primary version.
-->
  ...
  >
    ...
    <div style="display: flex; gap: 8px; margin-bottom: 20px">
      <button
        ...
        style="padding: 10px 24px; border-radius: 8px; border: none; background-color: #d9534f; color: #fff; font-size: 16px; cursor: pointer"
      >
        开始录音
      </button>
      <button
        v-else
        @click="stopRecording"
        style="padding: 10px 24px; border-radius: 8px; border: none; background-color: #5cb85c; color: #fff; font-size: 16px; cursor: pointer"
      >
        停止录音
      </button>
      <button
        @click="reset"
        style="padding: 10px 24px; border-radius: 8px; border: 1px solid #ccc; background-color: #fff; font-size: 16px; cursor: pointer"
      >
        重置
      </button>
    </div>
    ...
    <div
      v-if="partialText"
      style="padding: 12px; background-color: #f7f7f7; border-radius: 8px; margin-bottom: 12px"
    >
    ...
    <div
      v-if="preview"
      style="padding: 12px; background-color: #eef6ff; border-radius: 8px; margin-bottom: 12px"
    >
    ...
    <div
      v-if="error"
      style="padding: 12px; background-color: #fff0f0; border-radius: 8px; margin-bottom: 12px; color: #d9534f"
    >
    ...
    <div
      v-if="result"
      style="padding: 16px; background-color: #f0fff0; border-radius: 8px; margin-bottom: 12px"
    >
      <h3 style="margin-top: 0">翻译结果</h3>
      <p><strong>转录文字:</strong> {{ result.transcription }}</p>
      <p><strong>LLM推理:</strong> {{ result.reasoning }}</p>
      <div
      ...
      <div v-if="result.audioBase64" style="margin-top: 12px">
        <button
          ...
          style="padding: 8px 20px; border-radius: 6px; border: none; background-color: #5bc0de; color: #fff; font-size: 14px; cursor: pointer"
        >
