<!--
  Meowsformer — MeowPreviewPlayer Component (Legacy, root-level copy)
  See components/translate/MeowPreviewPlayer.vue for the primary version.
-->
...
const canConfirm = computed(() => hasListened.value && !props.isLoading);
const progressPct = computed(() =>
  duration.value > 0 ? (currentTime.value / duration.value) * 100 : 0
);
const shouldShow = computed(() => props.audioBase64 || props.isLoading);
</script>
...
<template>
  ...
  >
    ...
    <div v-if="previewDescription" class="meow-preview__description">
      <p class="meow-preview__summary">{{ previewDescription.summary }}</p>
      ...
      <details class="meow-preview__details">
        ...
        <pre class="meow-preview__detail-text">{{ previewDescription.detail }}</pre>
      </details>
    </div>
    ...
    <div v-if="audioBase64" class="meow-preview__player">
      <button
        ...
        :aria-label="playbackState === 'playing' ? '暂停预览' : '播放预览'"
      >
        {{
      ...
      <div class="meow-preview__progress" role="progressbar" :aria-valuenow="progressPct">
        <div class="meow-preview__progress-fill" :style="{ width: `${progressPct}%` }" />
    ...
    <div class="meow-preview__actions">
      <button
        ...
        :title="canConfirm ? '确认并发送此猫语' : '请先播放预览音频'"
      >
        ✓ 确认发送
      </button>
      <button
    ...
    <p v-if="!hasListened && audioBase64" class="meow-preview__hint">
